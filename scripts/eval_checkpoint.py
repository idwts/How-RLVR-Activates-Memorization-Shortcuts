...
def process(args: argparse.Namespace) -> None:
    ...
    model = LLM(args.model_path,
                tensor_parallel_size=4,  # 使用2张GPU
                gpu_memory_utilization=0.8,  # 控制GPU内存使用
                dtype="float16"
                )
    ...
    for dataset_name in args.datasets.split(","):
        ...
        # Calculate accuracies and update metadata
        all_accuracies = []
        verify_task = "math"
        ...
        # Update metadata with correctness (>= 50% correct)
        metadata = []
        for prompt, label, index in zip(formatted_prompts, all_labels, all_indices):
            metadata.append({
                ...
                "index": index,
                "outputs": [],
                "correct": None  # Add a placeholder for correctness
            })

        for outputs, acc in zip(all_outputs, all_accuracies):
            for idx, (output, is_correct) in enumerate(zip(outputs, acc)):
                metadata[idx]["outputs"].append(output.outputs[0].text)
                if metadata[idx]["correct"] is None:
                    metadata[idx]["correct"] = False
                if is_correct:
                    metadata[idx]["correct"] = True

        # for outputs, acc in zip(all_outputs, all_accuracies):
        #     for idx, (output, is_correct) in enumerate(zip(outputs, acc)):
        #         metadata[idx]["outputs"].append(output.outputs[0].text)
        #         if metadata[idx]["correct"] is None:  # Update correctness status
        #             # Check if the correct proportion is >= 50%
        #             correct_proportion = np.mean(acc)
        #             metadata[idx]["correct"] = bool(correct_proportion >= 0.5)
